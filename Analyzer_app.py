...
def show_statistics(df):
 ...
 st.subheader("굇굃굈굉굊 Descriptive Statistics (Pandas)")
 stats_df = df.describe().T
 st.dataframe(stats_df)
 st.subheader("脥껒껓껔껕 Additional NumPy Analysis (Range)")
...
def generate_plot(df, plot_type, col_x, col_y=None):
 ...
 st.subheader(f"굂굃굄 {plot_type} Visualization")
 ...
 if not col_x or not pd.api.types.is_categorical_dtype(df[col_x]) and not
pd.api.types.is_object_dtype(df[col_x]):
 ...
 if not col_x or not col_y or not pd.api.types.is_numeric_dtype(df[col_x]) or not
pd.api.types.is_numeric_dtype(df[col_y]):
...
======================================================================
===
st.title("Multi-Graph Data Analysis Web App 굇굃굈굉굊")
st.markdown("Upload a CSV file and choose from **Histograms**, **Bar Charts**, and
**Scatter Plots**.")
# --- File Uploader ---
...
if uploaded_file is not None:
 if st.session_state.df is None or uploaded_file.name !=
st.session_state.get('last_uploaded_name'):
...
or pd.api.types.is_object_dtype(df[col])]
 ...
 if plot_type == 'Histogram':
 col_x = st.selectbox("Select Numerical Column (X-axis)", options=numerical_cols if
numerical_cols else all_cols, key='hist_col_x')
 st.session_state.plot_col_x = col_x
 st.session_state.plot_col_y = None # Reset Y
 elif plot_type == 'Bar Chart':
 col_x = st.selectbox("Select Categorical Column (Categories)",
options=categorical_cols if categorical_cols else all_cols, key='bar_col_x')
 st.session_state.plot_col_x = col_x
 st.session_state.plot_col_y = None # Reset Y
 elif plot_type == 'Scatter Plot':
 c1, c2 = st.columns(2)
 with c1:
 col_x = st.selectbox("Select Numerical X Column", options=numerical_cols if
numerical_cols else all_cols, key='scatter_col_x')
 with c2:
 col_y = st.selectbox("Select Numerical Y Column", options=numerical_cols if
numerical_cols else all_cols, key='scatter_col_y')
 st.session_state.plot_col_x = col_x
